...
impl Source {
    /// Classify `text` per ADR-036's precedence: PR reference first (checked via
...
/// `Pr` resolution (ADR-036): reuse git-workon-lib's `pr.rs` PR workflow end-to-end, minus the
...
/// `merge-base(base tip, head tip)..head`, PR title carried through (ADR-036: "GitHub's own
...
mod tests {
    ...
    fn classify_bare_number_is_ref_not_pr() {
        // ADR-036 explicitly excludes a bare number — it could be a branch or an abbreviated
