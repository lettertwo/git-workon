...
//! overridable dispatch (see [ADR-034](../../../docs/adr/034-review-git-native-config-schema.md)).
//!
//! The pieces:
//! - [`Command`] — the enumerable set of *rebindable* actions. This is the compatibility surface
//!   ADR-034 calls out: each variant maps to a stable git-config action name (`stage-hunk`,
...
/// config (ADR-034's compatibility-surface consequence).
...
/// `default_keys` strings reproduce `tui.rs`'s exact pre-ADR-034 bindings — `Esc` is deliberately
...
/// (ADR-034: `space` is always the spacebar, never a chord of `s p a c e`). `None` for anything
/// not a reserved name.
...
/// Parse one whitespace-delimited token into a key sequence, per ADR-034's grammar:
...
/// or whitespace-only value yields no alternatives — an explicit *unbind* (ADR-034). Unparseable
/// tokens are skipped.
...
impl Keymap {
    /// The registry defaults with no config applied — the pre-ADR-034 hardcoded keymap. Used by
