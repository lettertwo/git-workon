...
/// carries only its capture index; its color is resolved HERE against `theme` (ADR-029's
...
fn content_spans(
    ...
    }) = emphasis
    {
        if is_word_pair {
            ...
            // full width — the line "is" the edit here (ADR-029's CS11 section: "edit" is the
...
mod tests {
    ...
    fn content_spans_syntax_mode_ignores_tint_fg_entirely() {
        // The changeset's primary gate (ADR-029 CS11): with `text_mode: Syntax`, `tint_fg` must
