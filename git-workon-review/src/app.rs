...
/// [`App::ensure_role_loaded`]'s combined branch and ADR-031's [`build_file_views`] (the loader
...
/// `workon.review.diff.text` (see ADR-029's "Revised (CS11, diff foreground/background split)"
...
/// [`DIFF_TEXT_OPTIONS`] — see [ADR-029](../../../docs/adr/029-review-theming-base16-hybrid.md)'s
...
pub struct ChangesetView {
    ...
    /// ADR-031's per-changeset acquisition state. `Ready` for every changeset this changeset
...
/// A [`ChangesetView`]'s acquisition state (ADR-031's "Slots" decision). `diff`/the `views_*`
...
impl ChangesetView {
    ...
    /// Construct a `Pending` slot for `cs` (ADR-031): no diff acquired yet. The outline shows
    ...
    /// Construct a `Failed` slot for `cs` carrying `message` (ADR-031): the acquisition attempt
    ...
    /// Whether this changeset's diff hasn't been acquired yet (ADR-031).
    pub fn is_pending(&self) -> bool {
        matches!(self.slot, ChangesetSlot::Pending)
    }

    /// Whether this changeset's acquisition attempt errored (ADR-031).
...
pub struct App {
    ...
    open_pending: bool,
    /// Whether a [`crate::app::FileLoadSpec`] has already been dispatched to the ADR-031 loader
    ...
    /// ADR-031's global generation counter. Invariant: bumps ⟺ every view cache was invalidated
...
impl App {
    ...
    /// ADR-031's global generation — see the field's doc comment for the invariant. The loader
    ...
    /// Whether the ACTIVE changeset's slot is `Pending` (ADR-031) — `render.rs`'s body path
    ...
    /// The ACTIVE changeset's failure message, if its slot is `Failed` (ADR-031) — `render.rs`'s
    ...
    // ── ADR-031: the loader thread's request/result seam ────────────────────────

    /// Snapshot everything the ADR-031 loader needs to load the CURRENT file, mirroring exactly
    ...
    /// (also zero files) will join this once ADR-031's later changesets land. Total by
    ...
    /// those cases, same non-fatal posture as the keymap/theme resolution (ADR-028).
...
impl DiffState {
    ...
    /// [`ChangesetView`] slots (ADR-031), which carry no real diff; existing `.diff.` read sites
...
// ── ADR-031: the loader thread's stateless request/job shape ────────────────────

/// Everything the ADR-031 loader job needs to reproduce one file's [`App::ensure_loaded`] work
...
/// byte-identical to an eager [`App::open_current`] — the invariant ADR-031 carries over from
...
mod tests {
    ...
    // ── ADR-031: the loader's request/result seam ────────────────────────────────
    ...
    // ── ADR-031: per-changeset slots (Pending/Ready/Failed) ─────────────────────
    ...
    fn app_is_constructible_from_a_pending_changeset_alone() {
        // ADR-031: `App::from_changesets`'s >=1 assert survives unchanged — an all-Pending stack
