//! Fixture tests for the M7 `Source` classifier + resolver (ADR-030): the `stack`/`uncommitted`
...
/// auto-detect (ADR-030: the uncommitted layer rides along). A dirty tree (an untracked file)
...
/// actually checked out, not the reviewed one (ADR-030).
#[test]
...
/// the source text (ADR-030's empty-but-valid UX, extended in CS3 to name the source).
#[test]
