...
//! rapid-outline-nav path, and (ADR-037) the streamed-startup path, driven against the real
...
//! - **Streamed startup (ADR-037):** before the progressive pipeline, a multi-changeset launch
...
/// ADR-037's Testing layer 3, part two: the `pty_responsiveness` extension the ADR calls for —
...
/// pre-ADR-037 multi-changeset path did) delays the event loop's first `recv_event` by the
...
/// measured numbers (streamed ~1.1-1.4s, reverted-to-pre-ADR-037 ~6.4s on this exact fixture)
