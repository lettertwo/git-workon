...
- **Theme overrides** (CS1, user-configurable colors tier — see
  [ADR-029](029-review-theming-base16-hybrid.md)'s CS1 revision) live in the `review.theme`
...
- **`reload-config` (`R`, global view, rebindable like any other action):** re-reads the
  ...
  `theme = auto`'s terminal-derivation probe (ADR-029) never re-runs mid-session — it needs
