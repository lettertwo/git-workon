...
Design locked 2026-07-07. Decisions live in **[ADR-028](../adr/028-review-git-native-config-schema.md)**
(config schema + keymap) and **[ADR-029](../adr/029-review-theming-base16-hybrid.md)**
...
- **Decision:** ADR-028. Replace the hardcoded `map_key` match (`tui.rs`) with a
...
- **Token-grammar parser** (ADR-028): reserved symbolic names (incl. `space`, `tab`, `enter`,
...
- **Decision:** ADR-029. Largest mechanical unit; **behavior-preserving** (dark stays
...
- Introduce `struct Base16 { base00..base0F }` / `Theme`. Re-express the current `render.rs`
  ...
  values ARE base08–0E + a ramp — see ADR-029). Derive diff tints via the existing
...
- Add the **light** base16 instance (paste a published base16 light scheme's 16 hexes — do
  NOT hand-invent; ADR-029). Wire `workon.review.theme` (via CS1) to select dark/light;
...
- **Decision:** ADR-029. The single most terminal-fragile unit — isolated on purpose.
...
- **Synthesize the 6 slots ANSI lacks** (base01/02/04/06/09/0F) per ADR-029's rules.
...
- `theme = <named>` / user-supplied base16 scheme files (the "user-configurable colors" tier).
  Additive later — the slot *source* is pluggable behind render-time resolution (ADR-029).
