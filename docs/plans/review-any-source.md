...
Design locked 2026-07-09. Decisions live in **[ADR-030](../adr/030-review-source-grammar.md)**
...
- **Lib seam**: `assemble_changesets` must be able to omit the uncommitted layer
  (ADR-030: layer only when focused on real HEAD). Prefer an explicit parameter over a
...
- `Ref` resolution, dispatched on shape (ADR-030): Graphite-tracked branch →
