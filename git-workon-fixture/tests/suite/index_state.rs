...
#[test]
fn bare_with_no_worktree_and_no_index_state_still_works() -> Result<(), Box<dyn std::error::Error>>
{
...
fn index_blob_equals_rejects_missing_path() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let repo = fixture.repo()?;
    assert!(!predicate::repo::index_blob_equals("missing.txt", b"anything".to_vec()).eval(repo));
...
fn workdir_file_equals_rejects_missing_path() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let repo = fixture.repo()?;
    assert!(!predicate::repo::workdir_file_equals("missing.txt", b"anything".to_vec()).eval(repo));
